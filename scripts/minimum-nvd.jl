...
# check if data/minimum_nvd/ exists, if not, create it
if !isdir("data/minimum_nvd/nvd")
    mkdir("data/minimum_nvd/nvd")
end

G = readIJ("data/minimum_nvd/minimum-network.csv"); # The network MUST be comma separated, with nodes as numerical indexes from 1 to n, without gaps

df = DataFrame(CSV.File("data/minimum_nvd/minimum-node_attributes.csv", delim=',', ignorerepeated=true))
df_names = names(df)
...
for i in range(2, size_of_names)
    ...
    output_file = "data/minimum_nvd/nvd/$(df_names[i]).csv"
